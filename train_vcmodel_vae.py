...
def log_validation(model, args, validation_transform, accelerator, global_step, vae):
    ...
    with torch.no_grad():
        for original_image in original_images:
            ...
            image = accelerator.unwrap_model(vae).decode(latents).sample
...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                reconstructed_image = accelerator.unwrap_model(vae).decode(latents).sample

            with accelerator.accumulate(model):
                # reconstruction loss. Pixel level differences between input vs output
                loss = 0
                mse_loss = F.mse_loss(latents, fmap) + F.l1_loss(latents, fmap) + F.mse_loss(reconstructed_image, pixel_values) + F.l1_loss(reconstructed_image, pixel_values)
