...
def log_validation(model, args, validation_transform, accelerator, global_step):
    ...
    for ori_img, img in zip(original_images, images):
        ...
        ori_img = torch.clamp(ori_img, 0.0, 1.0)
        img *= 255.0
        ori_img *= 255.0
...
def main():
    ...
    if args.discriminator_config_name_or_path is not None:
        # discriminator = Discriminator()
        discriminator = Discriminator.from_pretrained(args.discriminator_config_name_or_path)
    else:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if generator_step:
                with accelerator.accumulate(model):
                    ...
                    loss = 0
                    mseloss = F.mse_loss(pixel_values, fmap) + F.l1_loss(pixel_values, fmap)
                    loss += mseloss
                    ...
                    avg_mseloss = accelerator.gather(mseloss.repeat(args.train_batch_size)).float().mean()
                    avg_perceptual_loss = accelerator.gather(perceptual_loss.repeat(args.train_batch_size)).float().mean()
            ...
            if accelerator.sync_gradients and not generator_step and accelerator.is_main_process:
                ...
                if global_step % args.log_steps == 0:
                    ...
                    if avg_gen_loss is not None:
                        ...
                        logs["avg_mseloss"] = avg_mseloss.item()
                        logs["step_gen_loss"] = avg_gen_loss.item()
                        logs["avg_perceptual_loss"] = avg_perceptual_loss.item()
