...
def kl_divergence(reconstructed_tensor, groundtruth_tensor, path_size = 16, overlap=True, weight=1000):
    ...
    #         groundtruth = rearrange(groundtruth, "b c h w -> (b h w) c",)
    #     P = reconstructed.to(torch.float64)
    #     Q = groundtruth.to(torch.float64)
    #     cosine_sim = F.cosine_similarity(P, Q, dim=-1, eps=1e-9)
    #     cosine_dist = 1 - cosine_sim
    #     return cosine_dist.mean().to(dtype=dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                loss = 0
                mse_loss =F.mse_loss(pixel_values, fmap) * 1000 + F.l1_loss(pixel_values, fmap) * 50
                kld_loss = kl_divergence(pixel_values, fmap)
